...
const Doctors = () => {
  ...
  return (
    <div>
      ...
      <button className={`py-1 px-3 border rounded text-sm transition-all sm:hidden ${filter ? "bg-[#5f6fff] text-white" : " "}`} onClick={()=>setFilter(prev => !prev)}>Filters</button>
        <div className={`w-1/5 flex-col gap-4 text-sm text-gray-600 ${filter ? 'flex' : 'hidden sm:flex'}`}>
